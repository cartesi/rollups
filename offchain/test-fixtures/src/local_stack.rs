...
// On teardown, stops the container and removes it.
...
impl LocalStackFixture {
    pub async fn setup() -> LocalStackFixture {
        ...
        let args = ["-p", "4566:4566", "-v", VOLUME_PATH];
        let container_id =
            docker_cli::run("localstack/localstack", &args).await;
