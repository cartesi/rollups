...
async fn test_it_finishes_existing_epoch() {
    ...
    let expected_result = FinishEpochResponse {
        ...
        vouchers_epoch_root_hash: Some(grpc_client::Hash {
            data: vec![
                207, 39, 127, 184, 10, 130, 71, 132, 96, 232, 152, 133, 112, 183, 24, 241, 224,
                131, 206, 183, 111, 126, 39, 26, 26, 20, 151, 229, 151, 95, 83, 174,
            ],
        }),
...
async fn test_it_finishes_existing_epoch_with_outputs() {
    ...
    let vouchers_epoch_root_hash =
        decode_hash("29676ea41aaf54b4d66d45bc60b9c8f71b5f9166035d375626746e7396baa7a1");
    let notices_epoch_root_hash =
        decode_hash("63a367741b1feb9c2dc64bda8ac4a083ebbe5fd1f7bb4746e94597c988f30197");
...
async fn test_it_updates_input_index_after_finishing_epoch() {
    ...
    grpc_client
        .advance_state(grpc_client::create_advance_state_request(session_id, 1, 1))
        .await
        .unwrap();
...
fn assert_eq_finish_epoch_response(lhs: FinishEpochResponse, rhs: FinishEpochResponse) {
...
fn assert_eq_validity_proof(
    ...
    for (h_idx, hash) in lhs.output_hashes_in_epoch_siblings.into_iter().enumerate() {
