...
impl From<AdvanceMetadata> for HttpAdvanceMetadata {
    fn from(metadata: AdvanceMetadata) -> HttpAdvanceMetadata {
        HttpAdvanceMetadata {
            msg_sender: conversions::encode_ethereum_binary(
                &metadata.msg_sender,
            ),
...
impl From<RollupRequest> for HttpRollupRequest {
    fn from(request: RollupRequest) -> HttpRollupRequest {
        match request {
            RollupRequest::AdvanceState(request) => {
                HttpRollupRequest::AdvanceState {
                    data: request.into(),
                }
            }
            RollupRequest::InspectState(request) => {
                HttpRollupRequest::InspectState {
                    data: request.into(),
                }
            }
        }
...
impl TryFrom<HttpVoucher> for Voucher {
    ...
    fn try_from(voucher: HttpVoucher) -> Result<Voucher, VoucherDecodeError> {
        Ok(Voucher::new(
            conversions::decode_ethereum_binary(&voucher.destination)?
                .try_into()?,
...
pub enum VoucherDecodeError {
    #[snafu(display(
        "Invalid Ethereum address size (got {} bytes, expected 20 bytes)",
        got
    ))]
...
impl TryFrom<HttpRollupException> for RollupException {
    type Error = DecodeError;
    fn try_from(
        report: HttpRollupException,
    ) -> Result<RollupException, DecodeError> {
        Ok(RollupException {
            payload: conversions::decode_ethereum_binary(&report.payload)?,
        })
    }
...
impl TryFrom<HttpFinishRequest> for FinishStatus {
    ...
    fn try_from(
        request: HttpFinishRequest,
    ) -> Result<FinishStatus, DecodeStatusError> {
