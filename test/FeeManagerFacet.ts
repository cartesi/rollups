...
describe("FeeManager Facet", () => {
    ...
    it("test initial feePerClaim", async () => {
        expect(await feeManagerFacet.feeManagerDS.feePerClaim, "initial feePerClaim").to.equal(
            initialFeePerClaim
        );
    ...
    it("test constructor event FeeManagerCreated", async () => {
        let eventFilter = feeManagerFacet.filters.FeeManagerInitialized(
            null,
            null
        );
        let event = await feeManagerFacet.queryFilter(eventFilter);
