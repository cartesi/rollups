...
describe("Descartes V2 Implementation", ()=> {
  ...
  it("event DescartesV2Created", async ()=> {
    ...
    let eventFilter = descartesV2Impl.filters.DescartesV2Created(null,null,null,null,null,null);
    ...
    expect(     
      eventArgs['_input'],
      'input address'
    ).to.equal(mockInput.address);

    expect(     
      eventArgs['_output'],
      'output address'
    ).to.equal(mockOutput.address);
    ...
    expect(     
      eventArgs['_inputDuration'],
      'Input Duration'
    ).to.equal(inputDuration);

    expect(     
      eventArgs['_challengePeriod'],
      'Challenge Period'
