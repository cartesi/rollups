...
contract TestTwoPartyArbitrationEnum is Test {
    ...
    function test_enumOfInputPartition() public {
        ...
        assertTrue(
            TwoPartyArbitrationEnum.isInputPartitionVariant(
                context.state
            )
        );
    }
    ...
    function test_enumOfEpochHashSplit() public {
        TwoPartyArbitration.Context memory finalContext =
            createEpochHashSplit();

        assertTrue(
            TwoPartyArbitrationEnum.isEpochHashSplitVariant(
                finalContext.state
            )
        );
    }
    ...
    function test_enumOfMachineSplice() public {
        ...
        TwoPartyArbitration.Context memory machineSplice =
            TwoPartyArbitration.splitMachineDisagree(newEpochHashSplit);
            
        assertTrue(
            TwoPartyArbitrationEnum.isMachineSpliceVariant(
                machineSplice.state
            )
        );
    }
    ...
    */
    ...
    function createEpochHashSplit() internal view returns (TwoPartyArbitration.Context memory) {
        TwoPartyArbitration.ArbitrationArguments memory args =
            createSampleArbitrationArguments();
        args.initialHash = bytes32(0x04cde762ef08b6b6c5ded8e8c4c0b3f4e5c9ad7342c88fcc93681b4588b73f05);
        uint64 finalPoint_ = 4;
