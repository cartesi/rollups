...
// 0. to see what the value of payload and destination should be, uncomment the `console.log` statements
// 1. proofs are generated by the script `gen-proofs.sh` here[1]. You can specify `PAYLOAD` and `MSG_SENDER` in the script,
//    and add additional payloads as needed. The original script will be used to generate proof for 2 notices in epoch 0.
//    For vouchers, we modify the script to generate the 4 vouchers that we need.
//    Same raw payloads will be used in both notices and vouchers. But keep in mind that the encodings of notices and vouchers are different.
//    For Mac M1 users, use long duration of `sleep` command before `# Finish epoch`. For example, `sleep 10`.
// 2. `gen-proofs.sh` outputs a JSON file with proofs in base64 encoding. This tool[2] converts base64 to hex.
//    To install: `pip install base64-to-hex-converter`
//    To run: `python -m b64to16 proof.json`
// 3. import the decoded hex proof files for both notices and vouchers. And this test file will handle the rest.
// ref links:
