...
function buildSolCodes(p: OutputValidityProof): string {
    const lines : string[] = [
        ...
        '',
        '    constructor() {',
        `        proof.epochInputIndex = ${p.epochInputIndex};`,
        `        proof.outputIndex = ${p.outputIndex};`,
        `        proof.outputHashesRootHash = ${p.outputHashesRootHash};`,
        `        proof.vouchersEpochRootHash = ${p.vouchersEpochRootHash};`,
        `        proof.noticesEpochRootHash = ${p.noticesEpochRootHash};`,
        `        proof.machineStateHash = ${p.machineStateHash};`,
        `        uint256[${p.keccakInHashesSiblings.length}] memory array1 = [${p.keccakInHashesSiblings}];`,
        '        for (uint256 i; i < array1.length; ++i) {',
        '            proof.keccakInHashesSiblings.push(bytes32(array1[i]));',
        '        }',
        `        uint256[${p.outputHashesInEpochSiblings.length}] memory array2 = [${p.outputHashesInEpochSiblings}];`,
